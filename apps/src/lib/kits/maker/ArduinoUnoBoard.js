...
// Equivalent of playground-io for the Uno
function Uno(options) {
  ...
  if (!Uno.hasRegisteredSysexResponse) {
    ...
    Uno.hasRegisteredSysexResponse = true;
  }
}

Uno.prototype = Object.create(Firmata.board.prototype, {
  constructor: {
    value: Uno
  }
})
 ...
 */
export default class ArduinoUnoBoard extends EventEmitter {
  ...
  connectToFirmware() {
    return new Promise((resolve, reject) => {
      const serialPort = ArduinoUnoBoard.openSerialPort(this.portName_);
      const uno = ArduinoUnoBoard.makeUnoTransport(serialPort);
      const board = new five.Board({io: uno, repl: false, debug: false});
      board.once('ready', () => {
  ...
  initializeComponents() {
    ...
    return createArduinoUnoComponents(this.fiveBoard_).then(components => {
  ...
  destroy() {
    ...
    if (this.prewiredComponents_) {
      destroyArduinoUnoComponents(this.prewiredComponents_);
    }
    this.prewiredComponents_ = null;
  ...
  /**
   * Animate LED to demonstrate successful connection
   * to the board.
   * @returns {Promise} resolved when animation done.
   */
  celebrateSuccessfulConnection() {
    const { led } = this.prewiredComponents_;
     ...
     */
    function runFuncAndDelay(func, delay) {
      return new Promise(resolve => {
  ...
  /**
   * Create a Firmata controller attached to the given serial port.
   * @param {SerialPort} serialPort
   * @return {Uno}
   */
  static makeUnoTransport(serialPort) {
    const uno = new Uno({ port: serialPort });
    ...
    // Should see if this problem plagues my Uno's Firmata
    uno.on("open", function () {
      // Requesting the version requires both of these calls. ¯\_(ツ)_/¯
      uno.reportVersion(function () {});
      uno.queryFirmware(function () {});
    });
    return uno;
  }
}
